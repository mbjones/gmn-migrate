...
`sudo cat ~jones/.ssh/authorized_keys >> ~gmn/.ssh/authorized_keys`

## 2. set up an ssh tunnel to gmn host pg socket from the local machine

We'll set up a tunnel to the socket file so that the connection will be local rather
than over TCP, which allows the existing `local` access rules to be used to connect.
...
`pg-schema-diff plan --from-dsn "postgres://gmn@localhost:5555/gmn2" --include-schema "public" --temp-db-dsn "postgres://postgres:some_secure_pw@localhost:5432/postgres" --to-dsn "postgres://gmn@localhost:5555/gmn_arm" > gmn2-to-gmn-arm.sql`

## 5. Compare gmn2 to gmn_car-figshare as well
